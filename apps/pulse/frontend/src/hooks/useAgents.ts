...
export function useAgents() {
  ...
  const refresh = useCallback(async () => {
    try {
      const data = await get<{ agents: Agent[]; activeId: string }>('/api/agents')
  ...
  const switchAgent = async (id: string) => {
    // Set cookie first so server knows the agent even if POST fails
    document.cookie = `pulse_agent=${encodeURIComponent(id)};path=/;max-age=${30 * 86400}`
    setActiveId(id)
    try {
      await post('/api/agents/switch', { id })
    } catch {}
    // Always reload — switching agent changes all backend context
    window.location.reload()
  }

  const createAgent = async (name: string) => {
    const data = await post<{ ok: boolean; agent: Agent }>('/api/agents', { name })
    if (data.ok && data.agent) {
      setAgents(prev => [...prev, data.agent])
      setActiveId(data.agent.id)
      document.cookie = `pulse_agent=${encodeURIComponent(data.agent.id)};path=/;max-age=${30 * 86400}`
      // Reload to switch to new agent context
      window.location.reload()
    }
    return data.agent
  }
  ...
  const deleteAgent = async (id: string) => {
    ...
    setAgents(remaining)
    // If deleted agent was active, switch to first remaining or clear
    if (activeId === id) {

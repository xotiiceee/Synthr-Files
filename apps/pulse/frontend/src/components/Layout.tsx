...
import { MessageSquare, Bot, PenLine, Brain, BarChart3, Settings, Plus, Coins, LogOut, ChevronDown, Sun, Moon, TrendingUp, MessageCircle, X, Send, Check, Menu, Fingerprint, Image, ReceiptText, Search, ShieldCheck } from 'lucide-react'
...
const navItems = [
  ...
  { to: '/spend', label: 'Spend', icon: ReceiptText },
...
export default function Layout({ children }: { children: React.ReactNode }) {
  const {
    balanceUsd, projection, email, authProvider, isAdmin, logout,
    brands, selectedBrandId, selectedBrandName, switchBrand, createBrand,
  } = useAuth()
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg-0)' }}>
      ...
      <nav className={`pulse-sidebar${mobileOpen ? ' open' : ''}`} style={{
        ...
        <div style={{ padding: '0 12px 16px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <select
                ...
                style={{
                  ...
                  padding: '9px 32px 9px 12px', fontSize: 13, fontWeight: 500,
            ...
            {creatingBrand ? (
              <div style={{ display: 'flex', gap: 6 }}>
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') handleCreateBrand(); if (e.key === 'Escape') setCreatingBrand(false) }}
                  placeholder="Brand name..."
                  style={{
                    minWidth: 0, flex: 1, background: 'var(--select-bg)', color: 'var(--text-1)',
                    border: '1px solid var(--border)', borderRadius: 8,
                    padding: '7px 9px', fontSize: 12, outline: 'none', fontFamily: 'inherit',
                  }}
                />
                <button onClick={handleCreateBrand} style={{
                  width: 32, borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff',
              ...
              {activeAgent && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(activeAgent.name || 'A').slice(0, 1).toUpperCase()}
                  </div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ color: 'var(--text-1)', fontSize: 13, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {activeAgent.name}
                    </div>
                    <div style={{ color: 'var(--accent)', fontSize: 11, fontWeight: 650 }}>
                      Current agent
                    </div>
                  </div>
                </div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  style={{
                    width: '100%', background: 'var(--select-bg)', color: 'var(--text-1)',
                    border: '1px solid var(--border)', borderRadius: 8,
                    padding: '8px 10px 8px 30px', fontSize: 12, outline: 'none',
                    fontFamily: 'inherit',
                  }}
                />
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 188, overflowY: 'auto', paddingRight: 2 }}>
                {filteredAgents.map(agent => {
                  const isActive = agent.id === activeId
                  return (
                    <button
                      key={agent.id}
                      onClick={() => !isActive && switchAgent(agent.id)}
                      style={{
                        width: '100%',
                    ...
                    >
                      ...
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ color: 'var(--text-1)', fontSize: 12, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {agent.name}
                        </div>
                        <div style={{ color: 'var(--text-4)', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {agent.niche || agent.tone || 'Brand agent'}
                        </div>
                      </div>
                ...
                {filteredAgents.length === 0 && (
                  <div style={{ color: 'var(--text-4)', fontSize: 12, padding: '8px 4px' }}>
          ...
          {agents.length > 0 && !creating && (
            <div style={{ padding: '8px 8px 0' }}>
              <button
                ...
                onClick={() => setCreating(true)}
                style={{
                  width: '100%', padding: '7px 0', background: 'transparent', color: 'var(--text-3)',
                  border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, fontWeight: 500,
                  cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5,
                }}
              >
          ...
          {creating && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '8px 8px 0' }}>
              <input
                ref={inputRef}
                value={newName}
        ...
        <div style={{ padding: '0 12px 20px' }}>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {(() => {
              const isLow = balanceUsd != null && balanceUsd < 10;
              const isEmpty = balanceUsd != null && balanceUsd <= 0;
              ...
              return (
                <>
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ color: 'var(--text-2)', fontSize: 13, fontWeight: 500 }}>Balance</span>
                      </div>
                    ...
                    {isEmpty && (
                      <div style={{ marginTop: 8, fontSize: 12, color: 'var(--danger)', lineHeight: 1.4, fontWeight: 500 }}>
                        Balance empty — top up to continue.
                      </div>
                    )}
                    {isLow && !isEmpty && (
                    ...
                    {projection && balanceUsd != null && balanceUsd > 0 && projection.daysRemaining != null && projection.daysRemaining <= 14 && (
                      <div style={{
                  ...
                  >
                    {isEmpty || isLow ? 'Manage Balance' : 'View Spend'} <ReceiptText size={11} />
                  </NavLink>
                </>
              );
            })()}
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <div style={{
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <div style={{ color: 'var(--text-4)', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {isAdmin ? 'Admin session' : authProvider === 'demo' ? 'Built-in access mode' : 'Brand session'}
                  </div>
                </div>
              </div>
              <button

...
function ImageCard({ asset, onClick, onStar, onDelete }: {
  ...
  const imgSrc = asset.source.startsWith('http') || asset.source.startsWith('data:') ? asset.source : `/api/media-file/${asset.source}`
  return (
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <select value={model} onChange={(e) => setModel(e.target.value)} style={inputStyle}>
            <option value="fast">Fast (~$0.03)</option>
            <option value="quality">Quality (~$0.15)</option>
            <option value="freepik">Freepik Mystic (~$0.12)</option>
          </select>
        </div>
      </div>
      ...
      {result && (
        <div style={{ textAlign: 'center', marginTop: 12, marginBottom: 12 }}>
          ...
          <p style={{ color: 'var(--accent)', fontSize: 13, marginTop: 8 }}>
            Saved to library — ${(result.creditsUsed / 100).toFixed(2)} used
          </p>
        </div>
      )}

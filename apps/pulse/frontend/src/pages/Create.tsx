  ...
  const generate = async () => {
    ...
    try {
      ...
      if (e.message?.includes('Insufficient credits') || e.message?.includes('Not enough credits')) {
        setCreditError('Not enough credits. Top up at claw-net.org/dashboard to continue.')
      } else {
        ...
        <div style={{
          ...
          {lastCost != null && (
            <div style={{
              ...
              <span style={{ color: 'var(--text-3)' }}>Cost: <strong style={{ color: 'var(--accent)' }}>{lastCost} credits</strong></span>
              {creditsRemaining != null && (
                <span style={{ color: 'var(--text-4)' }}>Balance: {creditsRemaining.toLocaleString()} credits</span>
              )}
            </div>
          )}

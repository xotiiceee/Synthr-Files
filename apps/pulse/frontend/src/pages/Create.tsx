...
export default function Create() {
  ...
  const { agents, activeId } = useAgents()
  ...
  const [xPremium, setXPremium] = useState(() => localStorage.getItem('pulse_x_premium') === 'true')
  ...
  const [imagePickerMode, setImagePickerMode] = useState<'library' | 'generate'>('library')
  const [imagePrompt, setImagePrompt] = useState('')
  const [imageGenerating, setImageGenerating] = useState(false)
  const [imageError, setImageError] = useState('')
  const [generatedPreview, setGeneratedPreview] = useState('')
  const [useBrandCtx, setUseBrandCtx] = useState(false)
  ...
  useEffect(() => {
    get<{connected: boolean; handle: string | null}>('/api/x/status').then(s => {
      setXConnected(s.connected)
      if (s.connected) {
        get<{mentions: any[]}>('/api/x/mentions').then(m => setMentions(m.mentions)).catch(() => {})
      }
    }).catch(() => {})
  }, [])
  ...
  const generate = async () => {
    if (!topic.trim() || generating || (balanceUsd != null && balanceUsd <= 0)) return
    setGenerating(true)
    setPreview(null)
    try {
      const data = await post<{ ok: boolean; content: any; cost?: number; creditsRemaining?: number }>('/api/generate', {
        ...
        charLimit: charLimit,
        brandId: activeId,
        brandName: activeAgent?.brandName || activeAgent?.name,
        brandContext: activeAgent ? {
          name: activeAgent.name,
          brandName: activeAgent.brandName,
          niche: activeAgent.niche,
          tone: activeAgent.tone,
          topics: activeAgent.topics,
          competitors: activeAgent.competitors,
        } : undefined,
      })
      ...
      if (e.message?.includes('Insufficient credits') || e.message?.includes('Not enough credits')) {
        setCreditError('Not enough funds. Top up to continue.')
      } else {
  ...
  const charLimit = platform === 'x' && xPremium ? 25000 : activePlatform.charLimit
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 24px', minHeight: '100%', overflowY: 'auto', overflowX: 'hidden', boxSizing: 'border-box', wordBreak: 'break-word' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-3)', margin: 0 }}>Create and schedule content across platforms</p>
        </div>
      </div>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' }}>
              <div
                onClick={() => {
                  const next = !xPremium
                  setXPremium(next)
                  localStorage.setItem('pulse_x_premium', String(next))
                }}
                style={{
                  width: 38, height: 22, borderRadius: 11,
                  background: xPremium ? 'var(--accent)' : 'var(--bg-4)',
              ...
              >
                <div style={{
                  ...
                  left: xPremium ? 18 : 2, transition: 'left 0.2s',
                }} />
              </div>
              <span style={{ fontSize: 12, color: xPremium ? 'var(--accent)' : 'var(--text-3)', fontWeight: 600 }}>
                X Premium (25K chars)
              </span>
            </label>
          </div>
      ...
      <div style={{ ...cardStyle, marginTop: 8 }}>
        ...
        {creditError && (
          <div style={{
            ...
            <a href="https://claw-net.org/dashboard" target="_blank" rel="noreferrer" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              ...
              disabled={generating || !topic.trim() || (balanceUsd != null && balanceUsd <= 0)}
              style={{
                padding: '10px 22px', background: 'var(--accent)', color: '#fff',
                borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 600,
                cursor: generating || !topic.trim() || (balanceUsd != null && balanceUsd <= 0) ? 'not-allowed' : 'pointer',
                fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 8,
                opacity: generating || !topic.trim() || (balanceUsd != null && balanceUsd <= 0) ? 0.5 : 1, transition: 'all 0.15s',
              }}
            >
              {generating
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {estimatedCost > 0 && (
              <span style={{
                ...
                ~${(estimatedCost / 100).toFixed(2)} · {modelLabel}
        ...
        <div style={{
          ...
          {lastCost != null && (
            <div style={{
              ...
              <span style={{ color: 'var(--text-3)' }}>Cost: <strong style={{ color: 'var(--accent)' }}>${(lastCost / 100).toFixed(2)}</strong></span>
              {creditsRemaining != null && (
                <span style={{ color: 'var(--text-4)' }}>Balance: ${(creditsRemaining / 100).toFixed(2)}</span>
              )}
              {balanceUsd != null && balanceUsd <= 0 && (
                <span style={{ color: 'var(--danger)' }}>Add funds to generate new content.</span>
              )}
            </div>
          )}
          ...
          <div style={{ marginTop: 12, padding: 12, background: 'var(--bg-2)', borderRadius: 8, border: '1px solid var(--border)' }}>
            {attachedImage ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <img
                  src={attachedImage.source.startsWith('http') || attachedImage.source.startsWith('data:') ? attachedImage.source : `/api/media-file/${attachedImage.source}`}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={async () => {
                  ...
                  marginLeft: 8, padding: '4px 12px', borderRadius: 6, fontSize: 12,
                  border: '1px solid var(--accent)', background: 'var(--accent-dim)',
                  color: 'var(--accent)', cursor: 'pointer',
                }}>
                  <Sparkles size={12} style={{ marginRight: 4 }} /> AI Generate
                </button>
              </div>
            )}
          </div>
            ...
            <div onClick={() => setShowImagePicker(false)} style={{
              ...
              <div onClick={e => e.stopPropagation()} style={{
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
                  <button onClick={() => setImagePickerMode('library')} style={{
                    padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600,
                    border: imagePickerMode === 'library' ? '1px solid var(--accent)' : '1px solid var(--border)',
                    background: imagePickerMode === 'library' ? 'var(--accent-dim)' : 'var(--bg-2)',
                    color: imagePickerMode === 'library' ? 'var(--accent)' : 'var(--text-3)',
                    cursor: 'pointer', fontFamily: 'inherit',
                  }}>Library</button>
                  <button onClick={() => setImagePickerMode('generate')} style={{
                    padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600,
                    border: imagePickerMode === 'generate' ? '1px solid var(--accent)' : '1px solid var(--border)',
                    background: imagePickerMode === 'generate' ? 'var(--accent-dim)' : 'var(--bg-2)',
                    color: imagePickerMode === 'generate' ? 'var(--accent)' : 'var(--text-3)',
                    cursor: 'pointer', fontFamily: 'inherit',
                  }}><Sparkles size={12} style={{ marginRight: 4 }} />Generate</button>
                </div>
                {imagePickerMode === 'generate' ? (
                  <div>
                    {generatedPreview ? (
                      <div style={{ marginBottom: 16 }}>
                        <img src={generatedPreview} alt="Generated preview" style={{ width: '100%', maxHeight: 300, objectFit: 'contain', borderRadius: 12, border: '1px solid var(--card-border)', marginBottom: 12 }} />
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => {
                            setAttachedImage({ id: 'gen_' + Date.now(), name: 'AI Generated', source: generatedPreview, tags: imagePrompt.trim().split(' ').slice(0, 3) })
                            setShowImagePicker(false)
                            setGeneratedPreview('')
                            setImagePrompt('')
                          }} style={{
                            flex: 1, padding: '8px 14px', borderRadius: 8, border: '1px solid var(--accent)',
                            background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 13,
                          }}>Use this image</button>
                          <button onClick={() => { setGeneratedPreview(''); setImageError('') }} style={{
                            padding: '8px 14px', borderRadius: 8, border: '1px solid var(--border)',
                            background: 'var(--bg-2)', color: 'var(--text-3)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13,
                          }}>Try again</button>
                        </div>
                      </div>
                    ) : (
                        ...
                        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                          <input value={imagePrompt} onChange={e => setImagePrompt(e.target.value)}
                            ...
                            style={{
                              ...
                              background: 'var(--bg-1)', color: 'var(--text-1)', fontFamily: 'inherit', fontSize: 13,
                            }}
                            onKeyDown={e => {
                              if (e.key === 'Enter' && imagePrompt.trim()) {
                                ...
                                setImageError('')
                                setGeneratedPreview('')
                                post<{ok: boolean; imageUrl: string; creditsUsed: number}>('/api/media/generate', { prompt: imagePrompt.trim(), context: useBrandCtx ? 'brand' : undefined, size: '1024x1024' })
                                  .then(res => { if (res.ok) setGeneratedPreview(res.imageUrl) })
                                  .catch((e: any) => { setImageError(e.body?.error || 'Generation failed') })
                          ...
                          <button onClick={() => {
                            ...
                            post<{ok: boolean; imageUrl: string; creditsUsed: number}>('/api/media/generate', { prompt: imagePrompt.trim(), context: useBrandCtx ? 'brand' : undefined, size: '1024x1024' })
                              ...
                              .catch((e: any) => { setImageError(e.body?.error || 'Generation failed') })
                              .finally(() => setImageGenerating(false))
                          }} disabled={imageGenerating || !imagePrompt.trim()} style={{
                            padding: '10px 16px', borderRadius: 8, border: '1px solid var(--accent)',
                            background: 'var(--accent-dim)', color: 'var(--accent)',
                            cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 13,
                            whiteSpace: 'nowrap',
                          }}>{imageGenerating ? '...' : 'Generate'}</button>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12, color: 'var(--text-3)' }}>
                            <input type="checkbox" checked={useBrandCtx} onChange={e => setUseBrandCtx(e.target.checked)}
                              style={{ accentColor: 'var(--accent)' }} />
                            Use my brand context
                          </label>
                        </div>
                        <p style={{ fontSize: 11, color: 'var(--text-4)', margin: '0 0 12px' }}>$0.50 per image · Gemini 2.5 Flash via OpenRouter</p>
                        {imageError && <p style={{ fontSize: 12, color: '#ef4444', margin: '0 0 12px' }}>{imageError}</p>}
                      </>
                    )}
                  </div>
                ) : imagePickerLoading ? (
          ...
          {overLimit && (
            <div style={{
              ...
              color: '#ef4444', fontSize: 13, fontWeight: 500, lineHeight: 1.4,
            }}>
              Over the {activePlatform.label} limit ({charLimit} characters). Trim your content before posting.
            </div>
          )}
            ...
            {xConnected && (
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const text = editing ? editText : preview.text
                    await post('/api/x/post', { text, image_url: attachedImage?.source || undefined })
                    setXPosted(true)
                  } catch (e: any) {
                    setXPostError(e.body?.error || 'Failed to post')
                  }
                  setXPosting(false)
                }}
                disabled={xPosting || xPosted}
                style={{
                  padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600,
                  background: xPosted ? '#1d9bf020' : '#1d9bf0',
                  color: xPosted ? '#1d9bf0' : '#fff',
                  border: `1px solid ${xPosted ? '#1d9bf040' : '#1d9bf0'}`,
                  cursor: xPosting || xPosted ? 'default' : 'pointer', fontFamily: 'inherit',
                  display: 'flex', alignItems: 'center', gap: 6,
                }}
              >
                {xPosting ? <Loader2 size={14} className="spin" /> : xPosted ? <Check size={14} /> : <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>}
                {xPosted ? 'Posted' : `Post${xPosting ? 'ing' : ''} to X`}
              </button>
            )}
            {xPostError && <span style={{ fontSize: 12, color: '#ef4444', marginLeft: 8 }}>{xPostError}</span>}
            <button
              className="btn"
              onClick={generate}
              disabled={generating || (balanceUsd != null && balanceUsd <= 0)}
              style={{
                padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 500,
                background: 'var(--bg-2)', color: 'var(--text-2)', border: '1px solid var(--border)',
                cursor: generating || (balanceUsd != null && balanceUsd <= 0) ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                display: 'flex', alignItems: 'center', gap: 6,
                opacity: generating || (balanceUsd != null && balanceUsd <= 0) ? 0.5 : 1,
              }}
            >
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: scheduleDate ? 'var(--accent)' : 'var(--bg-3)',
                  color: scheduleDate ? '#fff' : 'var(--text-4)',
                  border: 'none', cursor: scheduleDate ? 'pointer' : 'not-allowed', fontFamily: 'inherit',
                }}
              >
                Confirm
              </button>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {(item.status === 'draft' || item.status === 'scheduled') && (
                      <button
                        ...
                        style={{ ...queueBtnStyle, background: 'var(--accent)', color: '#fff', border: 'none' }}
                      >
      ...
      {xConnected && mentions.length > 0 && (
        <div style={{ ...cardStyle, marginTop: 20 }}>
          <h3 style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 700, color: 'var(--text-1)' }}>Recent Mentions</h3>
          <div style={{ maxHeight: 300, overflowY: 'auto' }}>
            {mentions.map((m: any) => (
              <div key={m.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                <div style={{ fontSize: 13, color: 'var(--text-1)', lineHeight: 1.5, marginBottom: 6 }}>{m.text}</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 11, color: 'var(--text-4)' }}>{m.createdAt ? new Date(m.createdAt).toLocaleString() : ''}</span>
                  {replyToId === m.id ? (
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      <input value={replyText} onChange={e => setReplyText(e.target.value)}
                        placeholder="Reply..." style={{
                          padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)',
                          background: 'var(--bg-1)', color: 'var(--text-1)', fontFamily: 'inherit', fontSize: 12, width: 200,
                        }}
                        onKeyDown={e => {
                          if (e.key === 'Enter' && replyText.trim()) {
                            post('/api/x/reply', { text: replyText.trim(), tweet_id: m.id })
                              .then(() => { setReplyToId(null); setReplyText('') })
                              .catch(() => {})
                          }
                        }}
                      />
                      <button onClick={() => {
                        ...
                        padding: '6px 12px', borderRadius: 6, border: '1px solid #1d9bf0',
                        background: '#1d9bf0', color: '#fff', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, fontWeight: 600,
                      }}>Reply</button>
                    ...
                    <button onClick={() => { setReplyToId(m.id); setReplyText('') }} style={{

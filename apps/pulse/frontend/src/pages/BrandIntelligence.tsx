...
function EditableTag({ value, tagColors, onEdit, onRemove }: {
  ...
  return (
    <span style={{ ...baseTag, ...(tagColors || {}), cursor: 'default' }}>
      ...
      <X size={11} onClick={onRemove} style={{ cursor: 'pointer', opacity: 0.4, marginLeft: 2, transition: 'opacity 0.15s' }} onMouseEnter={(e: React.MouseEvent<SVGSVGElement>) => (e.currentTarget.style.opacity = '1')} onMouseLeave={(e: React.MouseEvent<SVGSVGElement>) => (e.currentTarget.style.opacity = '0.4')} />
    </span>
  )
}
...
const emptyProfile: BrandProfile = {
  identity: { name: '', tagline: '', description: '', keyFacts: [] },
  voice: { neverSay: [], signatures: [], toneNotes: '' },
  styleRules: { useHashtags: false, usePolls: false, emojiUsage: 'none', useStoryOpeners: false, customRules: [] },
  contentThemes: [],
  contentMix: { educational: 0.4, personal: 0.15, engagement: 0.25, promotional: 0.2 },
  learned: { topPerformers: [], bottomPerformers: [], bestHours: [], insights: [], updatedAt: undefined },
  updatedAt: '',
}
...
export default function BrandIntelligence() {
    ...
    (async () => {
      try {
        ...
        setProfile({ ...emptyProfile, ...data.profile,
          identity: { ...emptyProfile.identity, ...(data.profile?.identity ?? {}) },
          voice: { ...emptyProfile.voice, ...(data.profile?.voice ?? {}) },
          styleRules: { ...emptyProfile.styleRules, ...(data.profile?.styleRules ?? {}) },
          contentMix: { ...emptyProfile.contentMix, ...(data.profile?.contentMix ?? {}) },
          learned: { ...emptyProfile.learned, ...(data.profile?.learned ?? {}) },
        })
  ...
  const cycleEmoji = useCallback(() => {
    if (!profile) return
    const current = profile.styleRules?.emojiUsage ?? 'none'
    const idx = EMOJI_CYCLE.indexOf(current as typeof EMOJI_CYCLE[number])
    const next = EMOJI_CYCLE[(idx + 1) % EMOJI_CYCLE.length]
    save({ emojiUsage: next })
  }, [profile, save])

  const scanWebsite = useCallback(async () => {
    const website = scanUrl.trim()
    if (!website || scanLoading) return
    setScanLoading(true)
    setScanStatus(null)
    try {
      const res = await post<{ ok: boolean; profile: BrandProfile; domain: DomainKnowledge; scan?: { brand_name?: string } }>('/api/brand-profile/scan-website', {
        website,
      })
      if (res.profile) setProfile(res.profile)
      if (res.domain) setDomain(res.domain)
      setScanStatus(res.scan?.brand_name ? `Scanned ${res.scan.brand_name}` : 'Website scanned')
    } catch (err: any) {
      setScanStatus(err?.message || 'Website scan failed')
    } finally {
  ...
  if (!profile) {
    return (
      <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-3)' }}>
        ...
        <p style={{ fontSize: 15 }}>No brand profile yet. Create an agent to get started.</p>
  ...
  const hasVoice = profile.voice?.toneNotes || (profile.voice?.neverSay?.length ?? 0) > 0 || (profile.voice?.signatures?.length ?? 0) > 0
  const hasResearch = domain?.chunks && domain.chunks.length > 0
  const hasLearned = (profile.learned?.topPerformers?.length ?? 0) > 0 || (profile.learned?.insights?.length ?? 0) > 0 || (profile.learned?.bestHours?.length ?? 0) > 0
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 16px 60px', overflowX: 'hidden', boxSizing: 'border-box', wordBreak: 'break-word' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={cardStyle}>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-1)', margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Sparkles size={16} style={{ color: 'var(--accent)' }} /> Website Scan
          </h3>
          <p style={{ fontSize: 13, color: 'var(--text-3)', margin: '0 0 12px', lineHeight: 1.5 }}>
            Paste your site and Pulse will pull product details, positioning, tone, and themes into your brand profile.
          </p>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <input
              ...
              style={{ flex: '1 1 320px', minWidth: 240, ...baseTag, borderRadius: 8, padding: '10px 12px', background: 'var(--input-bg)', color: 'var(--text-1)' }}
            />
            <button
              ...
              style={{
                ...
                background: scanLoading || !scanUrl.trim() ? 'var(--bg-3)' : 'var(--accent)',
                color: '#fff', cursor: scanLoading || !scanUrl.trim() ? 'not-allowed' : 'pointer',
          ...
          {scanStatus && (
            <div style={{ marginTop: 10, fontSize: 12, color: scanStatus.toLowerCase().includes('failed') ? 'var(--danger)' : 'var(--accent)' }}>
              {scanStatus}
            </div>
          )}
        </div>
        ...
        <div style={cardStyle}>
          ...
          {hasVoice ? (
            <>
              ...
              {(profile.voice?.signatures?.length ?? 0) > 0 && (
                <div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            <span style={pillStyle(profile.styleRules?.useHashtags ?? false)} onClick={() => toggleStyle('useHashtags', profile.styleRules?.useHashtags ?? false)}>
              {profile.styleRules?.useHashtags ? '\u2713' : '\u2715'} Hashtags
            </span>
            <span style={pillStyle(profile.styleRules?.usePolls ?? false)} onClick={() => toggleStyle('usePolls', profile.styleRules?.usePolls ?? false)}>
              {profile.styleRules?.usePolls ? '\u2713' : '\u2715'} Polls
            </span>
            <span style={pillStyle(profile.styleRules?.useStoryOpeners ?? false)} onClick={() => toggleStyle('useStoryOpeners', profile.styleRules?.useStoryOpeners ?? false)}>
              {profile.styleRules?.useStoryOpeners ? '\u2713' : '\u2715'} Story Openers
            </span>
            <span style={{ ...pillStyle(false), cursor: 'pointer' }} onClick={cycleEmoji}>
              Emoji: {EMOJI_LABELS[profile.styleRules?.emojiUsage ?? 'none'] || 'None'}
            </span>
          </div>
          <div style={{ marginTop: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <MixBar label="Educational" value={profile.contentMix?.educational ?? 0.4} color="#10b981" />
          <MixBar label="Personal" value={profile.contentMix?.personal ?? 0.15} color="#8b5cf6" />
          <MixBar label="Engagement" value={profile.contentMix?.engagement ?? 0.25} color="#3b82f6" />
          <MixBar label="Promotional" value={profile.contentMix?.promotional ?? 0.2} color="#f59e0b" />
        ...
        {hasLearned && (
          <div style={cardStyle}>
            ...
            {(profile.learned?.topPerformers?.length ?? 0) > 0 && (
              <div style={{ marginBottom: 16 }}>
            ...
            {(profile.learned?.bottomPerformers?.length ?? 0) > 0 && (
              <div style={{ marginBottom: 16 }}>
            ...
            {(profile.learned?.bestHours?.length ?? 0) > 0 && (
              <div style={{ marginBottom: 16 }}>
            ...
            {(profile.learned?.insights?.length ?? 0) > 0 && (
              <div>

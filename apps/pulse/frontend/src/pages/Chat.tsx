...
export default function Chat() {
  const { refreshCredits, balanceUsd } = useAuth()
  ...
  const greeting: Message = {
    role: 'assistant',
    content: "Hey! I'm Pulse, your AI marketing assistant. I can set up your agent, adjust any settings, add knowledge, or help with strategy — all through conversation.\n\nWhat would you like to do?",
  }
  ...
  const sendMessage = async (overrideMsg?: string | React.MouseEvent) => {
    ...
    if (!msg || sending || (balanceUsd != null && balanceUsd <= 0)) return
    setInput('')
    setSending(true)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', maxWidth: 840, margin: '0 auto', padding: '0 24px', overflowX: 'hidden', boxSizing: 'border-box', wordBreak: 'break-word', minWidth: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-3)', margin: 0 }}>Set up and configure your agent through conversation</p>
          </div>
        </div>
        {messages.length > 3 && (
          <button
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 0', display: 'flex', flexDirection: 'column', gap: 20 }}>
        {messages.map((msg, i) => (
          <div key={i} style={{
            display: 'flex', gap: 12, width: '100%', minWidth: 0,
      ...
      <div style={{ borderTop: '1px solid var(--border-subtle)', padding: '16px 0 12px' }}>
            ...
            >
              {models.map(m => (
                <option key={m.id} value={m.id}>
                  {m.label} — {dollarsForCredits(m.credits)} / msg
                </option>
              ))}
            </select>
        ...
        <div style={{
          ...
          <button
            ...
            onClick={sendMessage}
            disabled={sending || !input.trim() || (balanceUsd != null && balanceUsd <= 0)}
            style={{
              width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: sending || !input.trim() || (balanceUsd != null && balanceUsd <= 0) ? 'var(--bg-3)' : 'var(--accent)',
              borderRadius: 12, border: 'none',
              cursor: sending || !input.trim() || (balanceUsd != null && balanceUsd <= 0) ? 'not-allowed' : 'pointer',
              color: '#fff', flexShrink: 0, transition: 'all 0.15s',
              opacity: sending || !input.trim() || (balanceUsd != null && balanceUsd <= 0) ? 0.4 : 1,
            }}
          >
        ...
        {balanceUsd != null && balanceUsd <= 0 && (
          <div style={{ marginTop: 8, color: 'var(--danger)', fontSize: 12 }}>
            Your balance is $0.00. Add funds to keep using chat.
          </div>
        )}

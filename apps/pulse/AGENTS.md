...
- Current `build-deploy.sh` only creates a local `deploy/` bundle. If no VPS target/credentialed deploy command is available, ask for the missing VPS host/path or deploy script instead of claiming deployment happened.
